  ...
  export const resolvers = {
    ...
    Mutation: {
      addBook: (parent: any, { book }: any, context: any, info: any) => {
        ...
        const newBook = {
          ...book,
          id: getAllBooks().length + 1,
          author: {id: book.authorId} // Generate a new ID
        };
        addBook(newBook);
        return newBook;
      },
    ...
    Author: {
      books: (author: {id: number; }, args: any, context: any, info: any) => {
        const authBooks = getBooksByAuthor(author.id);
        return authBooks;
      },
    }
  };
